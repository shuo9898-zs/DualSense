...
class CarlaSystem:
    ...
    def initialize(self):
        ...
        # Unreal rectangle: (8500,-17680), (8500,-16840), (7000,-17680), (7000,-16840) cm
        # CARLA: X=70.0~85.0 (m), Y=-176.8~-168.4 (m)
        self.driving_ui.add_work_zone(min_x=70.0, min_y=-176.8, max_x=85.0, max_y=-168.4)
    ...
    def _spawn_vehicle(self):
        ...
        target_location = carla.Location(x=243.0, y=-119.0)  # New position: Unreal (24300,-11900) cm
        ...
        # Rotate 180 degrees to reverse the vehicle heading.
        rotated_spawn = carla.Transform(
            closest_spawn.location,
            carla.Rotation(pitch=closest_spawn.rotation.pitch, 
                          yaw=closest_spawn.rotation.yaw + 180.0, 
                          roll=closest_spawn.rotation.roll)
        )
        
        try:
            self.vehicle = self.world.spawn_actor(vehicle_bp, rotated_spawn)
            print(f"✅ 车辆已生成在位置: ({rotated_spawn.location.x:.2f}, {rotated_spawn.location.y:.2f}, {rotated_spawn.location.z:.2f})")
            print(f"   朝向: {rotated_spawn.rotation.yaw:.2f}° (原始+180°)")
        ...
        def async_init_and_start():
            ...
            try:
                ...
                if timestamp_match:
                    ...
                    # Fallback option
                    gaze_dir = os.path.join(
                        r"C:\Users\USER\Desktop\CARLA_package\py code\35fps_Stable_Version\gaze_data",
