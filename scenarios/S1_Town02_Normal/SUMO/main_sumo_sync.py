...
class SumoCarlaSync:
    ...
    def __init__(self, carla_host='localhost', carla_port=2000, use_gui=False, save_path=None):
        ...
        # Configuration
        self.lateral_shift = -4.0  # Town02 lane-alignment adjustment
    ...
    def _add_ego_to_sumo(self):
        ...
        try:
            ...
            try:
                ...
                # Move to the current CARLA position.
    ...
    def _sync_carla_to_sumo(self):
        ...
        try:
            ...
            # Set speed first to avoid hard-braking warnings.
            traci.vehicle.setSpeed(self.ego_sumo_id, speed)
            
            # Update the SUMO ego position with moveToXY in unrestricted mode.
            traci.vehicle.moveToXY(
                vehID=self.ego_sumo_id,
            ...
            # Enforce unrestricted ego movement without boundary constraints.
            try:
                ...
                # Keep the vehicle even when outside the network.
                if not traci.vehicle.isOnRoad(self.ego_sumo_id):
                    logger.debug("Ego车辆不在路网中，但继续跟踪（正常情况）")
                    
            except Exception as safety_error:
                logger.debug(f"设置ego安全参数时警告: {safety_error}")
            
        except Exception as e:
            logger.warning(f"CARLA→SUMO同步错误: {e}")
